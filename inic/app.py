...
model = pd.read_pickle("inic/model_feliz.pkl")
...
with col2:
    ...
    area_opt = ['Biológicas', 'Exatas', 'Humanas']
    area = st.selectbox("Áreade formação",options= area_opt)
    posicao_opt = ['Iniciante','Junior', 'Pleno', 'Senior', 'Especialista', 'Coordenação',
       'Gêrencia', 'Diretoria', 'C-Level',]
    posicao = st.selectbox("Posição da cadeira", options=posicao_opt)
...
if proba > 0.7: 
    ...
    st.error(f"Você é uma pessoa infeliz! Probabilidade: {100* proba: .0f} %") 
